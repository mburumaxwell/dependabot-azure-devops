...
export function getBranch(): string | undefined {
  ...
  return process.env.GITHUB_REF_NAME || process.env.VERCEL_GIT_COMMIT_REF || getBranchFromGit();
}
...
export function getSha(): string | undefined {
  ...
  return process?.env.GITHUB_SHA || process?.env.VERCEL_GIT_COMMIT_SHA || getShaFromGit();
}
