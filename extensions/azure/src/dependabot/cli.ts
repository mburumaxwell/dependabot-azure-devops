...
export class DependabotCli {
   ...
   * @param jobToken - The job token to use for the update job.
   * @param operation
   * @param options
   * @returns
   */
    ...
    try {
      ...
      if (!existsSync(jobOutputPath) || (await stat(jobOutputPath))?.size == 0) {
        ...
        const env: Record<string, string | undefined> = {
          ...
          FAKE_API_PORT: options?.apiListeningPort?.toString(), // used to pin PORT of the Dependabot CLI api back-channel
        };
